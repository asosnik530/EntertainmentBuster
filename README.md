...
Link to tableau dashboard with visualizations: 
- https://public.tableau.com/profile/stacy.chandisingh#!/vizhome/NetViz/TheFinalStory?publish=yes
...
In order to create one final working dataset that contained all the variables we wanted, we needed to clean the source data and merge them all together using python and juptyer notebook. The cleaning was a lengthy process because the source data on kaggle was raw data that contained over 8000 rows with several streaming services and unnecessary fillers. Initially after the final merge, our country and genre columns had multiple values for each individual title so we simplified them to reduce overcrowding and complicated data so that it can be easily used for data analysis in python and tableau. Our primary final dataset contained our 3891 titles and all of their details like country of production, dates added to netflix, release years, ratings, IMDb scores, and genres.

#### Description of the analysis phase of the project:
We will be using Python and Tableau to create visualizations and tell a story with our data. 
...
#### Technologies, languages, tools, and algorithms used throughout the project:
 ...
 - Dashboard: Several software will be used to display our findings. We envision that we will have charts displaying the popularity of shows in comparison to key viewer demographics. 

#### Reccomendation for future analysis & anything the team would have done differently: 
Challenges we had included refining the model several times to ensure that there wasn’t multicollinearity between any of our independent variables. In the future, we would be interested in testing our model with data from other services like Disney + and Hulu, in addition to seeing how user factors such as age and gender affect program growth. 
